...
final class StatusItemController {
    ...
    private let quitItem = NSMenuItem(title: "Quit SayType", action: nil, keyEquivalent: "q")
    ...
    func update(phase: AppCoordinator.Phase, hotKey: KeyCombo, modelName: String?) {
        ...
        statusItem.button?.toolTip = "SayType (\(hotKey.displayString))"
    ...
    private func image(for phase: AppCoordinator.Phase) -> NSImage? {
        switch phase {
        case .idle:
            let image = NSImage(systemSymbolName: "mic", accessibilityDescription: "SayType idle")
            image?.isTemplate = true
            return image
        case .recording:
            let configuration = NSImage.SymbolConfiguration(paletteColors: [.systemRed])
            let image = NSImage(systemSymbolName: "mic.fill", accessibilityDescription: "SayType recording")?
                .withSymbolConfiguration(configuration)
            image?.isTemplate = false
            return image
        case .transcribing:
            let image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "SayType transcribing")
            image?.isTemplate = true
            return image
        }
    }
