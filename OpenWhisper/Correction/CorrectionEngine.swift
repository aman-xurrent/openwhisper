...
final class CorrectionEngine {
    ...
    private let queue = DispatchQueue(label: "com.amankumar.openwhisper.correction", qos: .userInitiated)
    ...
    init(modelURL: URL, modelName: String, useGPU: Bool) throws {
        ...
        guard let handle = openwhisper_llm_open(modelURL.path, useGPU, Self.contextTokens, threads) else {
    ...
    private func runCorrection(_ request: CorrectionRequest) throws -> String {
        ...
        let written = openwhisper_llm_complete(
            handle,
    ...
    private static let logHandlerInstallation: Void = {
        openwhisper_llm_set_log_callback({ level, message, _ in
