...
static openwhisper_llm_log_callback g_log_callback = NULL;
...
void openwhisper_llm_set_log_callback(openwhisper_llm_log_callback callback, void * user_data) {
    g_log_callback = callback;
    g_log_user_data = user_data;
    llama_log_set(forward_log, NULL);
}

openwhisper_llm * openwhisper_llm_open(const char * model_path, bool use_gpu, uint32_t context_tokens, int32_t threads) {
    ...
    openwhisper_llm * llm = calloc(1, sizeof(*llm));
    llm->model = model;
    llm->context = context;
...
void openwhisper_llm_close(openwhisper_llm * llm) {
    if (llm == NULL) {
        return;
    }
...
static char * render_prompt(openwhisper_llm * llm, const char * system_prompt, const char * user_prompt) {
...
static int32_t tokenize(openwhisper_llm * llm, const char * text, llama_token ** out_tokens) {
...
int32_t openwhisper_llm_complete(
    openwhisper_llm * llm,
